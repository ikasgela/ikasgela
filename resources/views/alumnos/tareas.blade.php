...
@section('content')
    ...
    <ul class="nav nav-tabs" id="pills-tab" role="tablist">
        {{-- Examen --}}
        @if($user->actividades_en_curso_autoavance()->enPlazoOrCorregida()->tag('examen')->count() > 0)
            <li class="nav-item">
                <a class="nav-link {{ $user->actividades_en_curso_autoavance()->enPlazoOrCorregida()->tag('examen')->count() > 0 ? 'active' : '' }}"
                   id="pills-examen-tab" data-toggle="tab" href="#pills-examen" role="tab"
                   aria-controls="pills-contact" aria-selected="false">{{ __('Exam') }}
                    <span
                        class="ml-2 badge badge-danger">{{ $user->actividades_en_curso_autoavance()->enPlazoOrCorregida()->tag('examen')->count() }}</span>
                </a>
            </li>
        @endif
        {{-- En curso --}}
    ...
    <div class="tab-content border-bottom border-left border-right" id="pills-tab-content">
        ...
        @if($user->actividades_en_curso_autoavance()->enPlazoOrCorregida()->tag('examen')->count() > 0)
            <div
                class="tab-pane fade {{ $user->actividades_en_curso_autoavance()->enPlazoOrCorregida()->tag('examen')->count() > 0 ? 'show active' : '' }}"
                id="pills-examen" role="tabpanel" aria-labelledby="pills-examen-tab">
                <div class="p-3">
                    @include('alumnos.partials.panel_actividades', ['actividades' => $user->actividades_en_curso_autoavance()->enPlazoOrCorregida()->tag('examen')->get(),
